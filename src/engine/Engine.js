...
import {
  ...
  StandardMaterial, DynamicTexture, Texture, Mesh
} from './blox3d.js';
...
export class Engine {
  ...
  setSky(preset = 'day') {
    const P = {
      day:    { img: 'assets/sky/day.jpg', fog: [0.62, 0.83, 0.93] },
      sunset: { img: 'assets/sky/sunset.jpg', fog: [0.91, 0.57, 0.42] },
      night:  { img: null, top: '#050a1e', mid: '#14224a', bot: '#2c3e6e', fog: [0.10, 0.15, 0.28] }
    }[preset] || {};

    const dome = MeshBuilder.CreateSphere('sky', { diameter: 960, segments: 20, sideOrientation: Mesh.BACKSIDE }, this.scene);
    const mat = new StandardMaterial('skymat', this.scene);
    if (P.img) {
      const tex = new Texture(P.img, this.scene);
